...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	for($i = 0; $i < count($friends); $i++){
		$friend = $friends[$i];
		$app = "Status Post";
		$note = $log_username.' posted on: <br /><a href="profile.php?u='.$account_name.'#status_'.$id.'">'.$account_name.'&#39;s Profile</a>';
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$app = "Status Reply";
		$note = $log_username.' commented here:<br /><a href="profile.php?u='.$account_name.'#status_'.$osid.'">Click here to view the conversation</a>';
